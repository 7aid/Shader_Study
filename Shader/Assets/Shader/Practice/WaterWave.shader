//水波
Shader "MyShader/Practice/WaterWave"
{
   Properties
    {
        //纹理
    ...
    {   //修改渲染队列为Transparent，但是RenderType渲染类型不修改，因为它本质上还是一个不透明物体
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i):SV_Target
            {
               ...
               fixed3 bump1 = UnpackNormal(tex2D(_BumpTex, i.uv.zw + speed)).rgb;
               fixed3 bump2 = UnpackNormal(tex2D(_BumpTex, i.uv.zw - speed)).rgb ;
               ...
               fixed3 wNormal = float3(dot(i.mulLine1.xyz, bump), dot(i.mulLine2.xyz, bump), dot(i.mulLine3.xyz, bump));
               //获取相对于法线的反射向量
